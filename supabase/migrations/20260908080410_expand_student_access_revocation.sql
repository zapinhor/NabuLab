...
begin
  ...
  if private.is_super_admin() then
    null;
  elsif p_user_id = v_actor_id then
    ...
    if v_target_role not in ('coordinator', 'teacher', 'student') then
      raise exception 'Diretor não pode remover outro Diretor' using errcode = '42501';
    end if;
  elsif private.has_org_role(
    p_organization_id,
    array['coordinator', 'teacher']::public.organization_role[]
  ) then
    if v_target_role <> 'student' then
      raise exception 'Coordenador e Professor só podem remover Alunos' using errcode = '42501';
    end if;
  else
